...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-c03d303a";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-c03d303a">'+'<span class="tile-face-c03d303a tile-front-c03d303a"></span>'+'<span class="tile-face-c03d303a tile-back-c03d303a"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-c03d303a")){return;}
var back=tile.querySelector(".tile-back-c03d303a");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-c03d303a");if(isMine){tile.classList.add("boom-c03d303a");}
if(dim){tile.classList.add("dimmed-c03d303a");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-c03d303a")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-c03d303a");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-c03d303a")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-c03d303a");void panelEl.offsetWidth;panelEl.classList.add("jolt-c03d303a");}
endRound();}
